...
EthernetSource::EthernetSource(uint16_t perListenerRouters, uint16_t intfid)
{
	...
	this->len 				= 0;
	this->intfId 			= intfid;
	this->intfName 			= IPGlobal::ETHERNET_INTERFACES[intfId];
	this->tIdx 				= 0;
	this->pTidx 			= 0;
...
void EthernetSource::packetReaderCallback(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
	...
	if(!IPGlobal::PKT_LISTENER_RUNNING_STATUS[intfId])
	{
	...
	tIdx = PKT_WRITE_TIME_INDEX(IPGlobal::CURRENT_EPOCH_SEC, IPGlobal::TIME_INDEX);
	...
	if(PKTStore::pktRepoCnt[intfId][ROUTER_TO_PROCESS][tIdx + 2 ] != 0)
		return;
...
void monitor(uint16_t interfaceId);

void* monitor_fn(void* arg)
{
  uint16_t intfId = *(uint16_t *)arg;
  monitor(intfId);
  return NULL;
}
...
void monitor(uint16_t interfaceId)
{
	...
	printf("  Ethernet Interface Stopped...\n");
